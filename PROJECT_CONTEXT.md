...
**Estado del proyecto:** READY — PRODUCTION DEPLOYED / VERIFIED / HARDENED (FASE 19 + 20) · Staging (Railway `pretty-motivaition`) DEPLOYED/VERIFIED/SECURE e intacto · Production aislada en project `feb-score-production` (env `production`, service `feb-score-api` + Postgres 18, dominio `https://feb-score-api-production.up.railway.app`, deployment `3471a1de` SUCCESS) · PITR + pg_dump offsite + restore drill (PITR 39s / lógico 7s) + rollback reales · API key production rotada (new PASS / old 401); `/docs` deshabilitado en prod via `FEB_SCORE_DISABLE_DOCS` · CD automated end-to-end validated (workflow run 31780767211 success) · CI verde (493 tests, FAB source connector B1/B2) · FEB-source connector delivered & merged & CI-green (FASE 21.B1/B2); real FEB format + dry-run; ingest real production POST PENDING FEB_TOKEN+FEB_API_KEY · Limitaciones: FEB ingesta REAL PENDING source/credential (conector entregado & idempotent); `OAUTH_INSUFFICIENT_GRANT` CLI backup schedule (PITR continuo opera); plan Free GitHub (required-reviewers → gate manual `confirm` input); production PAT→GitHub secret `RAILWAY_TOKEN` (manual); external S3 offsite (true DR) BLOCKED por falta de credentials; HSTS edge no configurablemitaciones: `OAUTH_INSUFFICIENT_GRANT` CLI backup schedule (PITR continuo opera); GitHub required-reviewers env protection (plan Free) → gate manual vía input `confirm`; production PAT→GitHub secret `RAILWAY_TOKEN` pendiente de creación manual; external S3 offsite (true DR) BLOCKED por falta de credentials; HSTS no configurable en Railway edge
