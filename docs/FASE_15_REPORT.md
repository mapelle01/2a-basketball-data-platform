...
**Estado final: READY WITH CONDITIONS — Docker CI verification pending** — fecha: 2026-08-13
...
(`/tmp/feb_audit/f15_operational.py`). Se encontraron y corrigieron 2 hallazgos
...
la verificación de imagen/compose queda como **condición externa pendiente** con
pipeline CI reproducible (`.github/workflows/ci.yml`) que la ejecutará en GitHub
Actions. Regresión final del suite: **473 tests, 0 FAIL / 0 XFAIL / 0 SKIP**,
`pip check` limpio.
...
| D-04 | INFO | Misma verificación de imagen Docker/compose `up` real no ejecutable aquí; handlers sincrónicos (psycopg bloqueante) corren en threadpool de Starlette — suficiente para carga de escritor único, cuello a considerar bajo alta concurrencia | §2/§11 del audit (25 concurrentes 200/200, 2 réplicas 200+409) | Pipeline CI (Docker) + documentación | Condición externa + observación |
...
- **Ejecución del CI en este entorno** (2026-08-13):
  ...
  - Job `docker` ejecutado en modo estático (Docker ausente):
    ...
    sustituye** el `docker build`/`compose up` real: sigue siendo la condición
    externa pendiente para GitHub Actions.
...
- **Docker CI verification pending**: imagen/compose/`up` real no verificados aquí
  (condición externa); cubiertos por el pipeline CI.
...
**READY WITH CONDITIONS — Docker CI verification pending.**
...
pasa con evidencia ejecutable (50/50) y sin regresiones (473/473). Los 2 hallazgos
LOW detectados se corrigieron con test reproducible. La única condición pendiente
es la verificación real de Docker/Compose, habilitada vía CI. No se avanza a
FASE 16.